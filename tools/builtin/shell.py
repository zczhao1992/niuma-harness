...
class ShellParams(BaseModel):
    ...
    timeout: int = Field(
        120, ge=1, le=600, description="超时时间(默认120秒)"
    )
    cwd: str | None = Field(
        None, description="命令的工作目录")
...
class ShellTool(Tool):
    ...
    async def execute(self, invocation: ToolInvocation) -> ToolResult:
        ...
        if not cwd.exists():
            return ToolResult.error_result(f"工作目录不存在: {cwd}")
        ...
        try:
            ...
            if sys.platform != "win32":
                ...
                process.kill()
            await process.wait()
            return ToolResult.error_result(f"命令在超时后终止 {params.timeout}s")
